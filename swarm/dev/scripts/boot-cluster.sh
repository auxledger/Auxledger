...
#
# The cluster contains a bootnode, a aux node and multiple swarm nodes, with
...
# start_bootnode starts a bootnode which is used to bootstrap the aux and
# swarm nodes
start_bootnode() {
...
# start_geth_node starts a aux node with --datadir pointing at <base-dir>/aux
# and a single, unlocked account with password "aux"
start_geth_node() {
  local dir="${base_dir}/aux"
  mkdir -p "${dir}"
  ...
  if [[ -z "${address}" ]]; then
    fail "failed to get aux account address"
  fi
  ...
  start_node "aux" "${GETH_IP}" "$(which aux)" ${args[@]}
}
...
create_account() {
  ...
  aux --datadir "${dir}" --password /dev/stdin account new <<< "${password}"
}
