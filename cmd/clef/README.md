Clef
----
Clef can be used to sign transactions and data and is meant as a replacement for aux's account management.
This allows DApps not to depend on aux's account management. When a DApp wants to sign data it can send the data to
...
The general flow for signing a transaction using e.g. aux is as follows:
![image](sign_flow.png)

In this case, `aux` would be started with `--externalsigner=http://localhost:8550` and would relay requests to `eth.sendTransaction`.
...
put together is a bit of a hack into the http server. This could probably be greatly improved
    - Relay: Geth should be started in `aux --external_signer localhost:8550`.
...
* Another potential thing to introduce is pairing.
  ...
  * Thus aux/mist/cpp would cryptographically handshake and afterwards the caller would be allowed to make signing requests.
